...
def test_optional_conversion() -> None:
    empty: Option[int] = from_optional(None)
...
def test_match_statement(opt_list: list[Option[int]]) -> None:
    for opt in opt_list:
        match opt:
...
def test_pipelining() -> None:
    ...
    assert_type(empty_piped, "Option[str]")
    assert empty_piped == Empty()

    some = make_option(5)
    some_piped = some | range_f | sum_typed | str
    assert_type(some_piped, "Option[str]")
